...
var	controlPage		=	"/stu/stu002m_01c2.jsp";	//控制頁面
...
function page_init()
{
	...
	Form.iniFormSet('EDIT', 'RESIDENCE_DATE', 'S',  8, 'M',  8, 'A');
	Form.iniFormSet('EDIT', 'NATIONCODE','R', 1,'D', 1, 'S',  1, 'M',  1, 'A');
	Form.iniFormSet('EDIT', 'NAME', 'R', 1, 'S', 10, 'M',  10, 'FS'); 
	Form.iniFormSet('EDIT', 'ENG_NAME', 'M', 26, 'S', 30, 'U');
	...
	*/
	...
	Form.iniFormSet('EDIT', 'TEL_OFFICE_EXT', 'S', 5, 'M',  6, 'A');
	...
	Form.iniFormSet('TAB2','NOU_EMAIL','R',1,'S',30);
	Form.iniFormSet('TAB2','DROPOUT_AYEAR','R',1,'N1','F', 3 ,'S',3,'M', 3);	
	Form.iniFormSet('TAB2','DROPOUT_SMS','D',1);
	//Form.iniFormSet('TAB2','STOP_PRVLG_SAYEARSMS','R',1,'S',10);
	//Form.iniFormSet('TAB2','STOP_PRVLG_EAYEARSMS','R',1,'S',10);
	...
	// Form.iniFormSet('TAB2','ENROLL_STATUS','R',1,'S',8);
	Form.iniFormSet('TAB2','SPECIAL_STTYPE_TYPE', 'M', 1);
	Form.iniFormSet('TAB2','SPECIAL_AYEAR', 'M', 3, 'S', 3, 'F', 3);
	Form.iniFormSet('TAB2','SPECIAL_SMS',1);
	Form.iniFormSet('TAB2','INFORMAL_STUTYPE', 'M', 1);
	_i('TAB2','SPECIAL_STTYPE_TYPE').onchange = function() {changeAyearSms();};
	Form.iniFormSet('TAB2', 'EDUBKGRD','M',1, 'S', 60, 'M',  100);	
	Form.iniFormSet('TAB2', 'EDUBKGRD_AYEAR','M',1, 'S', 3, 'M',  3);	
	Form.iniFormSet('TAB2', 'EDUBKGRD_GRADE','M',1, 'S', 1, 'M',  1);
...
//	Form.iniFormSet('EDIT', 'IDNO', 'AA', 'chkForm', '身分證字號');
	...
	//Form.iniFormSet('EDIT', 'MARRIAGE', 'AA', 'chkForm', '婚姻狀況');
	Form.iniFormSet('EDIT', 'VOCATION', 'AA', 'chkForm', '職業');
	Form.iniFormSet('EDIT', 'EDUBKGRD_GRADE', 'AA', 'chkForm', '學歷等級');
	//Form.iniFormSet('EDIT', 'DMSTADDR_ZIP', 'AA', 'chkForm', '戶籍地址郵遞區號');
	//Form.iniFormSet('EDIT', 'DMSTADDR', 'AA', 'chkForm', '戶籍地址');
	Form.iniFormSet('EDIT', 'CRRSADDR_ZIP', 'AA', 'chkForm', '通訊地址郵遞區號');
	Form.iniFormSet('EDIT', 'CRRSADDR', 'AA', 'chkForm', '通訊地址');
	//Form.iniFormSet('EDIT', 'AREACODE_HOME', 'AA', 'chkForm', '電話區碼(宅)');
	//Form.iniFormSet('EDIT', 'TEL_HOME', 'AA', 'chkForm', '電話(宅)');
	/*
	Form.iniFormSet('EDIT', 'EMRGNCY_NAME', 'AA', 'chkForm', '緊急聯絡人姓名');
	...
	*/
	...
	//Form.iniFormSet('EDIT', 'PASSPORT_NO', 'AA', 'chkForm', '護照號碼');
	//Form.iniFormSet('EDIT', 'FAX_TEL', 'AA', 'chkForm', '傳真電話');	
	...
	if(stno==''){		
		if(backTo=="1"){			
			top.mainFrame.location.href	='stu002m_01v1.jsp?backTo=1';
		}else{			
			top.mainFrame.location.href	='stu002m_01v1.jsp';
		}	
	}else{		
		top.stu002m_check="";
		top.detailFrame.location.href	='stu002m_01v1.jsp?stno='+stno;
		top.showDetail();
	}
	changeAyearSms();
}
...
function doSave()
{
	...
	if(document.EDIT.RESIDENCE_DATE.value != '' && document.EDIT.RESIDENCE_DATE.value.length != 8) {
		Form.errAppend("日期格式錯誤!!");
	}
	...
	Form.setInput("EDIT","EDUBKGRD_AYEAR",Form.getInput("TAB2","EDUBKGRD_AYEAR"));
	...
	Form.setInput("EDIT","INFORMAL_STUTYPE",Form.getInput("TAB2","INFORMAL_STUTYPE"));
	Form.setInput("EDIT","EDUBKGRD_GRADE_STUT003",Form.getInput("TAB2","EDUBKGRD_GRADE"));
	/** ================ */
...
// {
	// /** 開始處理 */
	// queryObj.startProcess();
	// loadind_.showLoadingBar (10, "準備鍵值資料");	
	...
	// /** 送到後端處理 */
	// var ajaxData = sendFormData("EDIT", controlPage, "EDIT_QUERY_MODE")	
	// if (ajaxData == null)
		// return;
	...
	// {
		...
		// {}
	// }	
	// ajaxData = sendFormData("EDIT", controlPage, "EDIT_QUERY_MODE2")
	// if (ajaxData == null)
		// return;
...
function doEdit_2(arguments)
{
    /** 開始處理 */	
	//by poto不知道為什麼　所以清掉~~
	Form.setInput("TAB2", "PRE_MAJOR_FACULTY","");
    Message.showProcess();
    ...
    var callBack1 = function doEdit_2.callBack1(ajaxData) {
		...
		Form.setInput("QUERY","STNO",Form.getInput("TAB2","STNO"));
		//by poto隱藏空大的資訊			
		if(_i('TAB2', 'STNO').value.length=='7'){				
...
function DEP_CHECK(DEP_CODE,SGU_CODE){		
	...
	if(DEP_CODE=='511'){			
	   Form.iniFormSet('EDIT', 'NAME', 'R', 1, 'S', 10, 'M',  10, 'FS'); 
	   Form.iniFormSet('EDIT', 'SPECIAL_MK', 'R', 0, 'D', 0); 	
	   Form.iniFormSet('TAB2', 'SPECIAL_STTYPE_TYPE', 'D', 0);			
	   Form.iniFormSet('TAB2', 'SPECIAL_AYEAR', 'D', 0);			
	   Form.iniFormSet('TAB2', 'SPECIAL_SMS', 'D', 0);
	   Form.iniFormSet('TAB2', 'INFORMAL_STUTYPE', 'D', 0);
	   Form.iniFormSet('TAB2', 'PRE_MAJOR_FACULTY', 'D', 0);
	}else if(DEP_CODE=='33'){		
			Form.iniFormSet('TAB2', 'SPECIAL_STTYPE_TYPE', 'D', 0);			
			Form.iniFormSet('TAB2', 'SPECIAL_AYEAR', 'D', 0);			
			Form.iniFormSet('TAB2', 'SPECIAL_SMS', 'D', 0);
			Form.iniFormSet('TAB2', 'INFORMAL_STUTYPE', 'D', 0);
			Form.iniFormSet('TAB2', 'STOP_PRVLG_SAYEAR', 'R', 1);
			Form.iniFormSet('TAB2', 'STOP_PRVLG_SSMS', 'D', 1);
			Form.iniFormSet('TAB2', 'STOP_PRVLG_EAYEAR', 'R', 1);
			Form.iniFormSet('TAB2', 'STOP_PRVLG_ESMS', 'D', 1);
			Form.iniFormSet('TAB2', 'PRE_MAJOR_FACULTY', 'D', 1);		
			Form.iniFormSet('EDIT', 'IDNO', 'R', 1);
			Form.iniFormSet('EDIT', 'BIRTHDATE', 'R', 1);
			Form.iniFormSet('EDIT', 'NAME', 'R', 1);
			Form.iniFormSet('EDIT', 'ENG_NAME', 'R', 1,'FS');
			Form.iniFormSet('EDIT', 'ALIAS',  'R', 1);
			Form.iniFormSet('EDIT', 'DMSTADDR_ZIP', 'R', 1);
			Form.iniFormSet('EDIT', 'DMSTADDR',  'R', 1);
			Form.iniFormSet('EDIT', 'CRRSADDR_ZIP',  'R', 1);
			Form.iniFormSet('EDIT', 'CRRSADDR',  'R', 1);
			Form.iniFormSet('EDIT', 'AREACODE_OFFICE',  'R', 1);
			Form.iniFormSet('EDIT', 'TEL_OFFICE','R', 1);
			Form.iniFormSet('EDIT', 'TEL_OFFICE_EXT', 'R', 1);
			Form.iniFormSet('EDIT', 'AREACODE_HOME', 'R', 1);
			Form.iniFormSet('EDIT', 'TEL_HOME',  'R', 1);
			Form.iniFormSet('EDIT', 'MOBILE',  'R', 1);
			Form.iniFormSet('EDIT', 'EMAIL', 'R', 1);
			Form.iniFormSet('EDIT', 'EMRGNCY_NAME', 'R', 1);
			Form.iniFormSet('EDIT', 'EMRGNCY_TEL', 'R', 1);
			Form.iniFormSet('EDIT', 'EMRGNCY_RELATION','R', 1);
			Form.iniFormSet('EDIT', 'PASSPORT_NO',  'R', 1);    
			Form.iniFormSet('EDIT', 'MARRIAGE', 'D', 1);
			Form.iniFormSet('EDIT', 'SEX', 'D', 1);
			Form.iniFormSet('EDIT', 'EDUBKGRD_GRADE', 'M', 1);
			Form.iniFormSet('EDIT', 'VOCATION', 'D', 1);
			Form.iniFormSet('EDIT', 'NATIONCODE', 'D', 1);			
			Form.iniFormSet('EDIT', 'FAX_TEL', 'R', 1);
			//Form.iniFormSet('EDIT', 'SAVE_BTN', 'D', 1);
			//Form.iniFormSet('EDIT', 'CLEAR_BTN', 'D', 1);
			Form.iniFormSet('EDIT', 'ADDR1', 'D', 1);
			Form.iniFormSet('EDIT', 'ADDR2', 'D', 1);
		//只能看
	}else if(DEP_CODE=='43'){
		//poto 2008/02/19		
		Form.iniFormSet('TAB2', 'STOP_PRVLG_SAYEAR', 'R', 1);
		...
		//名子不能改
			...
			Form.iniFormSet('EDIT', 'BIRTHDATE', 'R', 1);
			Form.iniFormSet('EDIT', 'NAME', 'R', 1);
			Form.iniFormSet('EDIT', 'ENG_NAME', 'R', 1);
			Form.iniFormSet('EDIT', 'ALIAS',  'R', 1);
			Form.iniFormSet('EDIT', 'DMSTADDR_ZIP', 'R', 1);
			Form.iniFormSet('EDIT', 'DMSTADDR',  'R', 1);
			Form.iniFormSet('EDIT', 'CRRSADDR_ZIP',  'R', 1);
			...
			Form.iniFormSet('EDIT', 'ADDR1', 'D', 1);
			Form.iniFormSet('EDIT', 'ADDR2', 'D', 1);
			//by poto 後來寫的註記 也不能修改 只能看
			Form.iniFormSet('TAB2', 'SPECIAL_STTYPE_TYPE', 'D', 1);			
			Form.iniFormSet('TAB2', 'SPECIAL_AYEAR', 'D', 1);			
			Form.iniFormSet('TAB2', 'SPECIAL_SMS', 'D', 1);	
			Form.iniFormSet('TAB2', 'INFORMAL_STUTYPE', 'D', 1);		
		//只能看
	}else{
		//by poto 2008/09/25  先檔起來
		Form.iniFormSet('TAB2', 'PRE_MAJOR_FACULTY', 'D', 1);	
	}
	
	var STTYPE=Form.getInput("TAB2","STTYPE");
	/*
	if(STTYPE=="全修生" && DEP_CODE!='43'){
		//Form.iniFormSet('TAB2', 'EDUBKGRD', 'D', 1,'R',1);
		//Form.iniFormSet('TAB2', 'EDUBKGRD_AYEAR', 'D', 1,'R',1);
	}else{
		if(DEP_CODE=='35'||DEP_CODE=='511'||DEP_CODE=='43'){
			//Form.iniFormSet('TAB2', 'EDUBKGRD', 'D', 0,'R',0);
			//Form.iniFormSet('TAB2', 'EDUBKGRD_AYEAR', 'D', 0,'R',0);
		}else{
			//Form.iniFormSet('TAB2', 'EDUBKGRD', 'D', 1,'R',1);
			//Form.iniFormSet('TAB2', 'EDUBKGRD_AYEAR', 'D', 1,'R',1);
		}
	...
	*/	
	if(DEP_CODE=='35'||DEP_CODE=='511'||DEP_CODE=='33'||DEP_CODE=='43'){
		//Form.iniFormSet('TAB2', 'EDUBKGRD', 'D', 1,'R',1); 
		//Form.iniFormSet('TAB2', 'EDUBKGRD_AYEAR', 'D', 1,'R',1);
	}else{
		//Form.iniFormSet('TAB2', 'EDUBKGRD', 'D', 1,'R',1);
...
function doSave1_end()
{
	...
	sendFormData("EDIT", controlPage, actionMode, callBack,false);
	changeAyearSms();
	doEdit_2(_i('EDIT', 'TMPV').value);	
}
